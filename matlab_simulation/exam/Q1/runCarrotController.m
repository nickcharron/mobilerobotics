function runCarrotController(pathPoints, L, r, v, x0, endLoc, figNum )
%% Description
...
%            disp(carrot(i-1,:))
    ...
    x(:,i) = x(:,i-1) + [ u(1,i)*cos(x(3,i-1))*dt;
                          ...
                          u(2,i)*dt] + error;
    %x(:,i) = x(:,i-1) + u(1,i)*[cos(x(3,i-1))*dt;sin(x(3,i-1))*dt;tan(u(2,i))/L*dt] + error ;  
...
% Plotting
    ...
    if(draw_iter == 10)
        drawbot(x(1,i-1),x(2,i-1),x(3,i-1),L,figNum)
        draw_iter = 0;
    end
